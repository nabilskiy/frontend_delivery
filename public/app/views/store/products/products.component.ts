...
import {StoreSetting} from "../setting/setting.component";
...
export class ProductsComponent implements OnInit {
    ...
    //schedule for item
    @ViewChild('addTimeModal')
    addTimeModal: ModalComponent;

    current_selected_day:number = 0;
    ...
    product_current_selected_day: number = 0;
    categoryTimePickerFrom: string;
    categoryTimePickerTo: string;
    product_schedule: any = [];
    product_schedule_form_open: boolean = false;
    ...
    constructor(public helper: Helper, public vcr: ViewContainerRef){
        ...
        this.data = {};
        this.item_types = this.helper.ITEM_TYPES;
    }
...
setTimeout(() => {
    ...
    open_edit_product_modal(product){
        ...
        this.product_schedule = this.product_data.product_schedule;
    ...
    updateProduct(product_data, event)
    {
        ...
        this.helper.http.post(this.helper.POST_METHOD.UPDATE_PRODUCT,json).map((res:Response) => res.json()).subscribe(res_data=>{
                ...
                if(res_data.success == true)
                {
                    ...
                    this.filtered_product_list[index].product_schedule = this.product_schedule;
                    this.edit_product_modal.close();
                    this.product_schedule = [];
                    this.product_current_selected_day = 0;
                    this.helper.message();
                }
                else
                {
        ...
        this.helper.http.post(this.helper.POST_METHOD.GET_STORE_PRODUCT_ITEM_LIST,{store_id:this.add_product.store_id, server_token:this.add_product.server_token,
            ...
            if(res_data.success){
                ...
                if(this.filtered_item_list.length>0){
                    ...
                    this.item_schedule = this.item_detail.item_schedule
                } else {
    ...
    updateItem(){
        ...
        this.item_detail.is_on_delivery = this.item_detail.is_on_delivery;
        this.item_detail.item_schedule = this.item_schedule;

        // const item_type_select = document.getElementById('#item_type_select');
        // console.log(item_type_select.value);
        // // this.item_detail.item_type = ;
    ...
    change_day(selected_day) {
        this.current_selected_day = selected_day;
    }

    open_add_time_modal()
    {
        this.timePickerError = null;
        this.addTimeModal.open();
    }

    remove_time() {
        this.item_schedule[this.current_selected_day] = null;
        this.updateItem();
    }

    saveNewItemTime()
    {
        const timeFromArr = this.timePickerFrom.split(':');
        const timeToArr = this.timePickerTo.split(':');

        if (timeFromArr[0] < timeToArr[0] || (timeFromArr[0] === timeToArr[0] && timeFromArr[1] < timeToArr[1])) {
            this.item_schedule[this.current_selected_day] = {
                dayNum: this.current_selected_day,
                from: this.timePickerFrom ? this.timePickerFrom : '',
                to: this.timePickerTo ? this.timePickerTo : ''
            };

            console.log('Save this time: ', this.item_detail);

            this.addTimeModal.close();
            this.updateItem();
        } else {
            this.timePickerError = "Час не коректний!";
        }
    }

//    product scheduler
    change_product_day(selected_day) {
        this.product_schedule_form_open = false;
        this.product_current_selected_day = selected_day;
    }

    saveNewProductTime()
    {
        const timeFromArr = this.categoryTimePickerFrom.split(':');
        const timeToArr = this.categoryTimePickerTo.split(':');

        if (timeFromArr[0] < timeToArr[0] || (timeFromArr[0] === timeToArr[0] && timeFromArr[1] < timeToArr[1])) {
            this.product_schedule[this.product_current_selected_day] = {
                dayNum: this.product_current_selected_day,
                from: this.categoryTimePickerFrom ? this.categoryTimePickerFrom : '',
                to: this.categoryTimePickerTo ? this.categoryTimePickerTo : ''
            };

            console.log('Save this time: ', this.product_schedule);
            console.log('Data: ', this.product_data);

            this.categoryTimePickerFrom = null;
            this.categoryTimePickerTo = null;

            this.openProductScheduleForm();
        } else {
            this.timePickerError = "Час не коректний!";
        }
    }
    ...
    remove_product_time() {
        this.product_schedule[this.product_current_selected_day] = null;
        this.product_schedule_form_open = false;
    }

    openProductScheduleForm() {
        this.timePickerError = null;
        this.product_schedule_form_open = !this.product_schedule_form_open;
    }
}
