...
export class StoreCreateOrderWithoutItemOrderComponent implements OnInit {
    ...
    add_to_cart(user_data){
        ...
        this.helper.http.post(this.helper.POST_METHOD.ADD_ITEM_IN_CART, json).map((res_data: Response) => res_data.json()).subscribe(res_data => {
            ...
            if(res_data.success){
                ...
                // this.helper.user_cart.cart_data.cart_id
    ...
    pay_order_payment(){
        ...
        const priceTotalNow = Number(this.product_total);
        const previousTotalPrice = Number(this.order_payment.total_cart_price);

        if (previousTotalPrice !== priceTotalNow) {
            this.product_total = priceTotalNow;
            this.get_distnce_time();
            this.form_field_validation_error = "Оновлення ціни, спробуйте ще раз після оновлення ціни!";
            return false;
        }
