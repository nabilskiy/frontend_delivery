...
@Component({
    selector: 'app-car_calling',
    templateUrl: './car_calling.component.html',
    providers: [Helper]
})
export class StoreCarCallingComponent implements OnInit {
    ...
    constructor(private helper:Helper, private location: Location, public vcr: ViewContainerRef, private router: Router) {
    ...
    ngAfterViewInit(){
        ...
        setTimeout(function () {
            ...
            jQuery('.chosen-container-single .chosen-default span').text('Обрати тип доставки');
        }, 1000);
    ...
    ngOnInit() {
        ...
        if (!store.use_car_calling) {
            this.router.navigate(['/store/create_order']);
        }
        ...
        jQuery(this.helper.elementRef.nativeElement).find('#selected_vehicle_id').on('change', (evnt, res_data) => {
            ...
            console.log('ngOnInit: => this.selected_vehicle_id ', this.selected_vehicle_id)
            jQuery('.chosen-container-single .chosen-default span').text('Обрати тип доставки');
            console.log('tyt->')
        });
    ...
    get_vehicle_list(){
        this.helper.http.post('/api/store/get_vehicles_list', {store_id: this.store_id,
            ...
            console.log('get_vehicle_list', res_data)
    ...
    get_order_invoice(google_distance, google_time){
        ...
        this.helper.http.post(this.helper.POST_METHOD.GET_ORDER_CART_INVOICE, get_order_cart_invoice_json).pipe(map((res_data: Response) => res_data.json())) .subscribe(res_data => {
                console.log('Set invoice - GET_ORDER_CART_INVOICE(res_data): ', res_data);
                this.myLoading = false;
                if(res_data.success){
                    this.order_payment = res_data.order_payment;
                    this.helper.user_cart.order_payment_id = res_data.order_payment._id;
                    this.helper.user_cart.cart_data.cart_id = res_data.order_payment.cart_id;
    ...
    create_order_service() {
        let json = {
            ...
            server_token: this.server_token,
            cart_id: this.helper.user_cart.cart_data.cart_id,
            // cart_id: this.order_payment.cart_id,
        ...
        this.helper.http.post(this.helper.POST_METHOD.STORE_CREATE_ORDER, json).pipe(map((res_data: Response) => res_data.json())).subscribe(res_data => {
            console.log('Create Order: ', res_data)
            this.myLoading = false;
            if(res_data.success){
