...
export class ProviderComponent implements OnInit {
    ...
    @ViewChild('provider_delete_modal')
    provider_delete_modal: ModalComponent;
    provider_should_deleted_id: any = null;
    ...
    open_delete_provider_modal(provider_id) {
        this.provider_should_deleted_id = provider_id;
        this.provider_delete_modal.open();
        console.log('ID -> ', provider_id);
    }

    deleteProvider() {
        console.log('deleted', this.provider_should_deleted_id);

        this.helper.http.post('/api/admin/delete_provider', {
            provider_id: this.provider_should_deleted_id,
        }).map((res) => res.json()).subscribe(res => {
            if (res.success) {
                this.provider_list = this.provider_list.filter(store => store._id !== this.provider_should_deleted_id);
                this.provider_should_deleted_id = null;
            }
        });

        this.provider_delete_modal.close();
    }
