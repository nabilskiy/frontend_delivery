...
export class ProfileComponent implements OnInit {
    ...
    @ViewChild('addCompanyModal')
    add_company_modal: ModalComponent;

    @ViewChild('otpVerifyModal')
    otp_verify_modal: ModalComponent;

    @ViewChild('cvvVerifyModal')
    cvv_verify_modal: ModalComponent;
    ...
    card_number_validation(evt){
        ...
        //         this.liqpay_card_data.card_type = card_type;
    ...
    verify_otp(otp){
        this.liqpay_otp = otp;
        this.otp_confirm_token = this.otp_confirm_token;
        // console.log("otp");
        // console.log(this.liqpay_otp);
        var data = {
            "store_id" : this.store_detail._id,
            "server_token" : this.store_detail.server_token,
            "otp": this.liqpay_otp.liqpay_otp,
            "confirm_token": this.otp_confirm_token
    }

        this.helper.http.post('/api/store/addcompanyverifyotp', data).pipe(map((response: Response) => response.json())) .subscribe(res_data => {
            if(res_data.success == true){
                this.otp_verify_modal.close();
                this.helper.data.storage = {
                ...
                this.otp_verify_modal.close();
                if(!res_data.verify.err_description){
                    this.helper.data.storage = {
                        "message": "Company can not be Add.",
                        "class": "alert-danger"
                    }
                }else{
                    this.helper.data.storage = {
                        "message": res_data.verify.err_description,
                        "class": "alert-danger"
                    }
                }
                this.helper.message();
    ...
    verify_cvv(cvv){
        ...
        var data = {
            "store_id" : this.store_detail._id,
            "server_token" : this.store_detail.server_token,
            "cvv": this.liqpay_cvv.liqpay_cvv,
            "confirm_token": this.otp_confirm_token
    }

        this.helper.http.post('/api/store/addcompanyverifycvv', data).pipe(map((response: Response) => response.json())) .subscribe(res_data => {
            if(res_data.success == true){
                this.cvv_verify_modal.close();
                this.helper.data.storage = {
                    "message": "Company added Successfully.",
                    "class": "alert-info"
                }
    ...
    add_company(){
        var card_data = {
                "store_id" : this.store_detail._id,
                "server_token" : this.store_detail.server_token,
                "card_no": this.add_card_data.card_number,
                "month": this.add_card_data.expiry_month,
                "year": this.add_card_data.expiry_year,
                "cvv": this.add_card_data.cvv
        }

        // console.log("asdsfsfererer");
        // console.log(card_data)
        this.helper.http.post('/api/store/addcompany', card_data).pipe(map((response: Response) => response.json())) .subscribe(res_data => {
            if(res_data.success == true){
                this.add_company_modal.close();
                this.helper.data.storage = {
                    "message": "Company added Successfully.",
                    "class": "alert-info"
                }
                ...
                console.log(res_data.verify);
                this.add_company_modal.close();
                this.otp_confirm_token = res_data.verify.confirm_token;

                if(res_data.verify.status == "otp_verify"){
                    this.otp_verify_modal.open();
                }else if(res_data.verify.status == "cvv_verify"){
                    this.cvv_verify_modal.open();
                }else{
                    if(!res_data.verify.err_description){
                        this.helper.data.storage = {
                            "message": "Company can not be Add.",
